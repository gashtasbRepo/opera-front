            ...
            <Chart
                ...
                pointOption={({ point, dataIndex }) => {
                    return {
                        key: point.date,
                        value: point.count,
                        style: { fill: feelTimeline.datas[dataIndex].color }
                    }
                }}
                datas={feelTimeline.data || []}
                keyAxis={{
                    gridLineColor: '#ddd',
                    size: 36,
                    getLabel: (index) => feelTimeline.labels[index],
                    count: feelTimeline.labels.length,
                    padding: [12, 12]
                }}
                valueAxis={{
                    start: 0,
                    end: feelTimeline.maxValue,
                    size: 48,
...
const useFeelTimeline = () => {
    const maxValue = 20
    const datas = [
        {title:'احساس 1',color:'rgb(255,0,0)'}, 
        {title:'احساس 2',color:'rgb(247,128,0)'}, 
        {title:'احساس 3',color:'rgb(238,255,0)'}, 
        {title:'احساس 4',color:'rgb(0,255,4)'}, 
        {title:'احساس 5',color:'rgb(0,8,255)'}
    ]
